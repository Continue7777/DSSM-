...
data_path = 'triple_pair.xlsx'
data_set = Data_set(data_path)
...
with tf.name_scope('Predict'):
    ...
    pred_prod = tf.reduce_sum(tf.multiply(query_y, doc_y), 1, True)
    pred_norm_prod = tf.multiply(query_norm, doc_norm)

    # cos_sim_raw = query * doc / (||query|| * ||doc||)
    pred_cos_sim_raw = tf.truediv(prod, norm_prod)

    pred_cos_sim = tf.transpose(tf.reshape(tf.transpose(cos_sim_raw), [MAIN_QUESTION_NUM, 1]))
    
    pred_prob = tf.nn.softmax(cos_sim)
    # 只取第一列，即正样本列概率。
    pred_label = tf.argmax(pred_prob)
...
def pull_all(index_list):
    ...
    query_in = data_set.get_onehot_from_batch(index_list,'query')
    doc_positive_in = data_set.get_onehot_from_batch(index_list,'main_question')
    doc_negative_in = data_set.get_onehot_from_batch(index_list,'wrong_question')
