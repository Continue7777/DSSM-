...
flags.DEFINE_integer('step_num', 10000, 'batch_step')
...
with tf.Session(config=config) as sess:
    ...
    for batch_id in range(FLAGS.step_num):       
        if batch_id % FLAGS.print_cycle == 0 and batch_id != 0:
            ...
            summary_v,_ = sess.run([merged,train_step], feed_dict=feed_dict_train(True)) 
            train_writer.add_summary(summary_v, batch_id)
        else:
            sess.run(train_step, feed_dict=feed_dict_train(True))
